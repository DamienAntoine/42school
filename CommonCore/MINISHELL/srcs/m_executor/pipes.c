	...
	while (i < num_commands)
	{
		...
		if (pid == 0) // Child process
		{
			...
			if (i < num_commands - 1) // If not the last command
				dup2(pipes[i][1], STDOUT_FILENO); // Set output to the next pipe

			// Close all pipe ends not used by this process
			for (j = 0; j < num_commands - 1; j++)
			{
				close(pipes[j][0]);
				close(pipes[j][1]);
			...
			// Set up redirection (after pipes, so it takes priority)
